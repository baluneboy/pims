...
def sum_otorunhist_pickle_files(pickle_files, tag, ax_cols=None):
...
def get_percentile_index(csum_pct, pctile, axs='xyzv'):
    """Return index of first cumulative sum percentage value >= input percentile value."""
    idxs = list()
    for ia, a in enumerate(axs):
        # print ia, a
        idxs.append(np.argwhere((csum_pct[:, ia] >= pctile)).flatten()[0])
    return idxs
...
def get_percentile(log10rms_bin_centers, csum_pct, pctile, axs='xyzv'):
    ...
    # get index values for each ax in axs
    idxs_pctile = get_percentile_index(csum_pct, pctile, axs=axs)
