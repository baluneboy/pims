...
# UEZE13RT1387C ER8 Locker 5 Current for POLAR 1
# UEZE13RT1388C ER8 Locker 6 Current for POLAR 2 
# UEZE13RT1391C ER8 Locker 7 Current for POLAR 3
# UEZE05RT1388C ER5 Locker 6 Current for POLAR 4
# UEZE06RT1392C ER6 Locker 8 Current for POLAR 5
# UGZG25RT2172T Temperature for POLAR 1
# UGZG27RT2172T Temperature for POLAR 2
# UGZG28RT2172T Temperature for POLAR 3
#               Temperature for POLAR 4
#               Temperature for POLAR 5

POLAR_MSID_MAP = {
        'UEZE13RT1387C': 'ER8_Locker_5_Current_for_POLAR_1',
        'UEZE13RT1388C': 'ER8_Locker_6_Current_for_POLAR_2',
        'UEZE13RT1391C': 'ER8_Locker_7_Current_for_POLAR_3',
        'UEZE05RT1388C': 'ER5_Locker_6_Current_for_POLAR_4',
        'UEZE06RT1392C': 'ER6_Locker_8_Current_for_POLAR_5',
        'UGZG25RT2172T': 'Temperature_for_POLAR_1',
        'UGZG27RT2172T': 'Temperature_for_POLAR_2',
        'UGZG28RT2172T': 'Temperature_for_POLAR_3',
        }

MSID_TEMPLATES = {
    '/misc/yoda/www/plots/batch/padtimes/NRT_sto_files/polar/polar.sto': POLAR_MSID_MAP,
}
...
def drawers_sto2dataframe(stofile, msid_map=POLAR_MSID_MAP):
    """read ascii sto file into dataframe for POLAR current and temperatures"""
    s = StringIO()
...
def old_main():
    ...
    tdir1 = os.path.join(tdir, 'Drawer1')
    tdir2 = os.path.join(tdir, 'Drawer2')    
    plot_data(bname, df, 'ER8_Locker_5_Current_for_POLAR_1', 'Current (A)', -0.1, 1.1, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=cdir1)
    plot_data(bname, df, 'ER8_Locker_6_Current_for_POLAR_2', -0.1, 1.1, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=cdir2)
    plot_data(bname, df, 'Temperature_for_POLAR_1', 'Temp. (C)',    20,  28, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=tdir1)
    plot_data(bname, df, 'Temperature_for_POLAR_2', 'Temp. (C)',    20,  28, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=tdir2)
    

def jaxa_request(sto_file):
   
    this_map = get_msid_map(sto_file)
    bname = os.path.basename(sto_file).replace('.sto', '')    
    df = drawers_sto2dataframe(sto_file, msid_map=this_map)
    df.to_csv('/tmp/polar.csv')
    ROT_DEG = 20
    INT_SEC = None  # None or 300
    matplotlib.rc('xtick', labelsize=8)
    pdir = '/misc/yoda/www/plots/user/handbook/source_docs/hb_vib_equipment_POLAR_2018-07-03'
    cdir = os.path.join(pdir, 'current')
    tdir = os.path.join(pdir, 'temperature')
    plot_data(bname, df, 'ER5_Locker_6_Current_for_POLAR_4', 'Current (A)', -0.1, 5.1, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=cdir)
    # plot_data(bname, df, 'ER8_Locker_5_Current_for_POLAR_1', 'Current (A)', -0.1, 1.1, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=cdir)
    # plot_data(bname, df, 'ER8_Locker_6_Current_for_POLAR_2', 'Current (A)', -0.1, 1.1, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=cdir)
    # plot_data(bname, df, 'Temperature_for_POLAR_1', 'Temp. (C)',    20,  28, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=tdir)
    # plot_data(bname, df, 'Temperature_for_POLAR_2', 'Temp. (C)',    20,  28, interval_sec=INT_SEC, rot_deg=ROT_DEG, out_dir=tdir)

if __name__ == '__main__':
    """ASSUMED MSID MAP AND no rolling mean here, plot just for short/few hours span"""

    sto_file = '/misc/yoda/www/plots/batch/padtimes/NRT_sto_files/polar/polar.sto'  # no thinning
    sto_file = '/misc/yoda/www/plots/batch/padtimes/NRT_sto_files/polar/polar16.sto'  # no thinning
    jaxa_request(sto_file)
    sys.exit('bye')
    ...
    ROT_DEG = 20
    INT_SEC = None  # None for no rolling mean; 300 for 5-minute rolling mean
    matplotlib.rc('xtick', labelsize=8)
