...
log = SimpleLog('test_stream', log_level='INFO').log

class StreamTestCase(unittest.TestCase):
    """
    Test suite for obspy.core.stream.PadStream.
    """
    ...
    def test_setitem(self):
        """
        Tests the __setitem__ method of the PadStream object.
        """
    ...
    def test_getitem(self):
        """
        Tests the __getitem__ method of the PadStream object.
        """
        stream = read()
    ...
    def test_iadd(self):
        """
        Tests the __iadd__ method of the PadStream objects.
        """
        stream = self.mseed_stream
        self.assertEqual(4, len(stream))
        other_stream = self.gse2_stream
    ...
    def test_addTraceToStream(self):
        """
        Tests using a Trace on __add__ and __iadd__ methods of the PadStream.
        """
        st0 = read()
        st1 = st0[0:2]
        tr = st0[2]
    ...
    def test_append(self):
        """
        Tests the append method of the PadStream object.
        """
        stream = self.mseed_stream
        # Check current count of traces
        self.assertEqual(len(stream), 4)
        # Append first traces to the PadStream object.
    ...
    def test_extend(self):
        """
        Tests the extend method of the PadStream object.
        """
        stream = self.mseed_stream
        # Check current count of traces
        self.assertEqual(len(stream), 4)
        # Extend the PadStream object with the first two traces.
    ...
    def test_insert(self):
        """
        Tests the insert Method of the PadStream object.
        """
        stream = self.mseed_stream
        self.assertEqual(4, len(stream))
    ...
    def test_getGaps(self):
        """
        Tests the getGaps method of the PadStream objects.
    ...
    def test_pop(self):
        """
        Test the pop method of the PadStream object.
        """
        stream = self.mseed_stream
    ...
    def test_slicing(self):
        """
        Tests the __getslice__ method of the PadStream object.
        """
        stream = read()
        padstream = PadStream(traces=stream.traces)
        self.assertEqual(stream[0:], stream[0:])
        self.assertEqual(stream[:2], stream[:2])
        self.assertEqual(stream[:], stream[:])
        self.assertEqual(len(stream), 3)
        new_stream = padstream[1:3]
        self.assertTrue(isinstance(new_stream, PadStream))
    ...
    def test_slicingWithStep(self):
        """
        Tests the __getslice__ method of the PadStream object with step.
        """
        tr1 = Trace()
        tr2 = Trace()
        tr3 = Trace()
        tr4 = Trace()
        tr5 = Trace()
        st = PadStream([tr1, tr2, tr3, tr4, tr5])
    ...
    def test_cutout(self):
        """
        Test cutout method of the PadStream object. Compare against equivalent
        trimming operations.
        """
    ...
    def test_remove(self):
        """
        Tests the remove method of the PadStream object.
        """
        stream = self.mseed_stream
        # Make a copy of the Traces.
        stream2 = deepcopy(stream)
        # Use the remove method of the PadStream object and of the list of Traces.
    ...
    def test_reverse(self):
        """
        Tests the reverse method of the PadStream object.
        """
        stream = self.mseed_stream
        # Make a copy of the Traces.
        traces = deepcopy(stream[:])
        # Use reversing of the PadStream object and of the list.
        stream.reverse()
        traces.reverse()
        # Compare all Traces.
        self.assertEqual(4, len(stream))
    ...
    def test_select(self):
        """
        Tests the select method of the PadStream object.
        """
        ...
        for header in headers:
            traces.append(Trace(data=np.random.randint(0, 1000, 100),
                                header=header))
        stream = PadStream(traces=traces)
        # Test cases:
    ...
    def test_sort(self):
        """
        Tests the sort method of the PadStream object.
        """
        ...
        # Use normal sorting.
        stream.sort(keys=['network', 'station', 'location', 'channel', 'starttime', 'endtime'], reverse=True)
        self.assertEqual([i.stats.sampling_rate for i in stream.traces],
    ...
    def test_mergeWithDifferentCalibrationFactors(self):
        """
        Test the merge method of the PadStream object.
        """
    ...
    def test_mergeGaps2(self):
        """
        Test the merge method of the PadStream object on two traces with a gap in
        between.
        """
    ...
    def test_split(self):
        ...
        """
        # 1 - create a PadStream with gaps
        tr1 = Trace(data=np.ones(4, dtype=np.int32) * 1)
        tr2 = Trace(data=np.ones(3, dtype=np.int32) * 5)
        tr2.stats.starttime = tr1.stats.starttime + 9
        st = PadStream([tr1, tr2])
    ...
    def test_bugfixMergeMultipleTraces2(self):
        ...
        st = PadStream([trace1, trace2, trace3])
        st.merge()
    ...
    def test_mergeWithSmallSamplingRate(self):
        ...
        # create stream
        st = PadStream([trace1, trace2, trace3, trace4])
        # merge
        st.merge()
        # compare results
    ...
    def test_trimRemovingEmptyTraces(self):
        ...
        stream = PadStream([trace1, trace2, trace3])
        stream.trim(UTCDateTime(900), UTCDateTime(1100))
        # Check if only trace2 is still in the PadStream object.
    ...
    def test_pickle(self):
        """
        Testing pickling of PadStream objects..
        """
    ...
    def test_comparisons(self):
        ...
        st0 = PadStream([tr0])
        st1 = PadStream([tr1])
        st2 = PadStream([tr0, tr1])
        st3 = PadStream([tr2, tr3])
        st4 = PadStream([tr1, tr2, tr3])
        st5 = PadStream([tr4, tr5, tr6])
        st6 = PadStream([tr0, tr6])
        st7 = PadStream([tr1, tr7])
        st8 = PadStream([tr7, tr1])
        st9 = PadStream()
        stA = PadStream()
        ...
        for st in [st0, st1, st2, st3, st4, st5, st6, st7, st8]:
            self.assertEqual(stA == st, False)
            self.assertEqual(stA != st, True)
        # some weird tests against non-PadStream objects
    ...
    def test_str(self):
        ...
        st = PadStream([tr1, tr2])
        result = st.__str__()
        expected = "2 Trace(s) in PadStream:\n" + \
    ...
    def test_cleanup(self):
        ...
        for trB in [tr2, tr3, tr4]:
            trA = tr1.copy()
            st = PadStream([trA, trB])
            st._cleanup()
            self.assertTrue(st == PadStream([tr1]))
        ...
        for trB in [tr7, tr8, tr9, tr10, tr11, tr12, tr13]:
            ...
            with warnings.catch_warnings(record=True):
                ...
                st._cleanup()
            self.assertTrue(st == PadStream([trA, trB]))
    ...
    def test_copy(self):
        """
        Testing the copy method of the PadStream object.
        """
        st = read()
        st2 = st.copy()
