...
#            print 'append() is moving %s to %s, success:%s' % (self._fileName_, newName, ok)
        ...
        txyzs = packet.txyz()
        # FIXME to investigate rotation, change xyz vector to like (1,0,0) or (0,1,0) or (0,0,1) here
        ...
        if  self._rotateData_ and 4 == len(atxyzs[0]):  # do coordinate system rotation
            ...
            print "AFTER", atxyzs[1,1:]; raise SystemExit            
...
def addAncillaryXML(db, entry, dataTime, sensor, pw, dbMatchTime): 
    ...
    if db == 'bias':
        ...
        if pw.setDataCoordSystem(dataName, dataTime, sensor):
            #print pw._rotationMatrix_; raise SystemExit
...
#~/dev/programs/python/pims/pad/packetwritertransform/packetWriterFromChef.py tables=121f05 cutoffDelay=0 delete=0 ancillaryHost=kyle startTime=1455705600 endTime=1455706200
