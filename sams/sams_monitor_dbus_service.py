...
usage = """Usage: python sams_monitor_dbus_service.py &
# ALSO START AclockScreenlet.py"""
...
# FIXME this is fake status! (SEE OTHER FIXME BELOW AND IN AclockScreenlet.py)
...
class SamsMonitor(dbus.service.Object):

    # FIXME mimic HelloWorld for SamsStatus BUT do not print hello_message
    # FIXME and in SamsStatus method get actual status and not deque/fake one
    ...
    def update(self):
        for class_name in query_list:
            obj = get_obj(class_name, self, host, schema, uname, pword)
            obj.register()
    ...
    @dbus.service.method("gov.nasa.grc.pims.SamsInterface",
                         in_signature='', out_signature='as')
    def GetStatus(self):
        #sams_status = d.popleft()
        #d.append(sams_status)
        self.update()
        sams_status = self.status
        return [sams_status, " from sams_monitor_dbus_service.py", "with unique name",
                session_bus.get_unique_name(), 'at', str(datetime.datetime.now()) ]
    ...
    def GetTuple(self):
        return ("Hello Tuple", " from sams_monitor_dbus_service.py")

    @dbus.service.method("gov.nasa.grc.pims.SamsInterface",
                         in_signature='', out_signature='a{ss}')
    def GetDict(self):
        return {"first": "Hello Dict", "second": " from sams_monitor_dbus_service.py"}
...
def process_config_file(sams_monitor, host, schema, uname, pword, query_list):
    for class_name in query_list:
        obj = get_obj(class_name, sams_monitor, host, schema, uname, pword)
        obj.register()
    sams_monitor.update()
...
class DemoException(dbus.DBusException):
    _dbus_error_name = 'gov.nasa.grc.pims.DemoException'
...
if __name__ == '__main__':
    ...
    if not parametersOK(): raise SystemExit
    
    #sams_monitor = SamsMonitor()
    config_file = parameters['config_file']
    validator_file = parameters['validator_file']
    refresh_minutes = parameters['refresh_minutes']
    host, schema, uname, pword, query_list = get_config(config_file, validator_file)

    dbus.mainloop.glib.DBusGMainLoop(set_as_default=True)

    session_bus = dbus.SessionBus()
    name = dbus.service.BusName("gov.nasa.grc.pims.SamsService", session_bus)
    object = SamsMonitor(session_bus, '/SamsMonitor')

    mainloop = gobject.MainLoop()
    print "Running SAMS monitor service."
    print usage
    mainloop.run()
        
    #while True:
    #    process_config_file(sams_monitor, host, schema, uname, pword, query_list)
    #    time.sleep(60.0*refresh_minutes)
