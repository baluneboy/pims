...
def file_trapz_pad_files(start, stop, sensor, fs, fc, taghours):
    ...
    # TODO maybe at some point we will handle multiples, but not here yet
    if len(taghours) != 1:
    ...
    for d in pd.date_range(start, stop):
        ...
        if len(day_files) == 0:
            ...
            for pad_file in ffp(day_files):
                ...
                # print amag_stats
                c += 1
            print 'processed %d files' % c, fpmaxs,
        print ''

    return fpmaxs, np.concatenate(fsums, axis=0)


def get_10sec_integration_bins(gmin=-5.0e-04, gmax=5.0e-04, bin_width=1.0e-07):
    """Return g-level bin width, edges, centers and count for 10-second integral (velocity)."""
...
def hist_pickle_files(pickle_files):
    """Return xyz per-axis hist_counts from histogram of fsums in pickle files.

    The pickle files were to have been generated by save_transient_pickle routine.
    """

    # initialize running values for per-axis histograms of fsums
    hist_counts = my_zeros((num_log10rms_bins - 1, 3))

    # iterate over pickle files plucking values to add to histogram counts
    for f in pickle_files:
        # load data from this pickle file
        data = pkl.load(f)
        # FIXME we do not need to iterate over columns -- RIGHT?  JUST USE NUMPY'S COLUMNWISE wisdom
        for idx, c in enumerate(ax_cols):
            hist_counts[:, idx] += np.histogram(data, log10rms_bin_edges)[0]  # idx=0 bc no need for 2nd (bins) return value
...
def save_transient_pickle(start, stop, sensor, fsums, fpmaxs, taghours, out_dir='/misc/yoda/www/plots/batch/results/transient'):
    ...
    pickle_file = os.path.join(out_dir, out_name)
...
if __name__ == '__main__':
    ...
    #                                                    hour-range
    #                                                         vv vv
    # ./main.py -s SENSOR    -d STARTDATE  -e STOPDATE   -t TAG1,h1,h2
    # ./main.py -s 121f03006 -d 2016-01-01 -e 2016-01-31 -t sleep,0,4

    ################################################################
    # EX#2 Sum/combine transient pickle files to produce histograms
    # ./main.py -s SENSOR    -d STARTDATE  -e STOPDATE -t sleep --plot
    # ./main.py -s 121f03006 -d 2017-01-01 -e 2017-03-30 -t sleep --plot
    ...
    if args.fromfile is not None:
        ...
        if args.plot:
            ...
            raise Exception('FIXME: in a hurry for gateway, so skipping "plot" processing type [NEEDS WORK], for now')
            ...
            save_transient_pickle(args.start, args.stop, args.sensor, fsums, crude_amag_stats, args.taghours)
