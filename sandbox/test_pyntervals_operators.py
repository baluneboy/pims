...
from intervals import FloatInterval, IllegalArgument, IntInterval
from pyntervals import MyDateInterval as DateInterval


class TestComparisonOperators(object):
    @mark.parametrize(('comparison', 'result'), (
        (DateInterval([date(2019, 1, 1), date(2019, 2, 1)]).length == timedelta(days=31), True),
        (DateInterval([date(2019, 1, 1), date(2019, 2, 1)]).radius == timedelta(days=31)/2, True),
